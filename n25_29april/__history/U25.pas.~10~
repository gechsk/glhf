...
uses
  ...
  Vcl.ExtCtrls, Vcl.Menus;
   ...
   type FF = record
    X1,X2,X3,Y1,Y2,Y3: real; // исходные данные
    P,S:real; // результат
    end;
    var
    calculations: array[1..128] of FF;
...
AssignFile(f,Fname);
Rewrite(f);
 s1:=Form1.X1.lines[0]; //Считываем
 s2:=Form1.X2.lines[0];
 s3:=Form1.X3.lines[0];
 s4:=Form1.Y1.lines[0];
 s5:=Form1.Y2.lines[0];
 s6:=Form1.Y3.lines[0];
 Writeln(f,s1); //заполняем файл
 Writeln(f,s2);
 Writeln(f,s3);
 Writeln(f,s4);
 Writeln(f,s5);
...
Form1.TSG.Cells[1,1]:=FormatFloat('#####.###',PR);//ограничение количества знаков после запятой
Form1.TSG.Cells[1,2]:=FormatFloat('#####.###',PL);
  ...
  Procedure InitialFillingInTheTable; //заполнение обозначений в таблице
  begin
